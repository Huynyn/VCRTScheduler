...
import { FileDown, Trophy, Users, UserX, Check, X, ShieldCheck } from 'lucide-react';
...
function KeptApartPanel({ schedule, avoidancePairs, nameById }) {
  if (!avoidancePairs || avoidancePairs.length === 0) return null;

  const sharedByPair = new Map();
  for (const v of schedule.metrics?.issues?.avoidanceViolations || []) {
    sharedByPair.set(pairKey(makePair(v.pair[0], v.pair[1])), v.slots.length);
  }

  const keptCount = avoidancePairs.filter(
    ([a, b]) => !sharedByPair.get(pairKey(makePair(a, b)))
  ).length;

  return (
    <div className="mb-4 rounded-lg border border-gray-200 overflow-hidden">
      <div className="bg-gray-50 px-3 py-2 text-sm font-semibold text-secondary-700 flex items-center gap-2">
        <UserX size={15} className="text-primary-500" /> Kept apart
        <span className="ml-auto text-xs font-medium text-gray-500">
          {keptCount}/{avoidancePairs.length} kept apart
        </span>
      </div>
      <ul className="divide-y divide-gray-100">
        {avoidancePairs.map(([a, b]) => {
          const key = pairKey(makePair(a, b));
          const names = `${nameById[a] || 'Unknown'} & ${nameById[b] || 'Unknown'}`;
          const shared = sharedByPair.get(key) || 0;
          const apart = shared === 0;
          return (
            <li key={key} className="px-3 py-2 flex items-start gap-2 text-sm">
              {apart ? (
                <ShieldCheck size={15} className="mt-0.5 shrink-0 text-success-600" />
              ) : (
                <X size={15} className="mt-0.5 shrink-0 text-danger-500" />
              )}
              <div>
...
export default function ScheduleResults({ result }) {
  const { responders, avoidancePairs } = useResponders();
      ...
      <CardBody>
        ...
        {!schedule.valid && (
          <div className="alert-warning mb-3 text-sm">
            ...
            {schedule.metrics.underHours > 0 &&
              `${schedule.metrics.underHours} responder(s) below their committed weekly hours. `}
        ...
        <KeptApartPanel schedule={schedule} avoidancePairs={avoidancePairs} nameById={nameById} />
