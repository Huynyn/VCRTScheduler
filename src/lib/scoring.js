...
const committedHours = (r) => (r.hours === REDUCED_HOURS ? REDUCED_HOURS : FULL_HOURS);
...
const UNDER_HOURS_PENALTY = 70000; // per responder scheduled below their committed weekly hours

// Keep-apart ("avoid together") is a HARD rule: a pair that must be kept apart
...
export function evaluate(assignment, responders, avoidancePairs = [], preferredPairs = []) {
  ...
  const underHoursIds = responders
    ...
    .map((r) => r.id);
  const underHours = underHoursIds.length;
  ...
  const score =
    ...
    underHours * UNDER_HOURS_PENALTY -
