...
export function buildPatterns(responder) {
  ...
  if (unique.length === 0) {
    // A 12h responder MUST work a full 12h week \u2014 either one overnight or two
    // day shifts. If their availability can't form one, we do NOT silently drop
    // them to a single 6h shift: that breaks their weekly commitment (and would
    // read as a valid schedule when it isn't). Instead we return no pattern with
    // a clear reason, so the feasibility report flags them and the coordinator
    // knows exactly who needs to open up more availability to reach 12h.
    //
    ...
    const usableDayCount = [...usable].filter(isDaySlot).length;
    if (usableDayCount === 1 && [...usable].every(isDaySlot)) {
      return {
        patterns: [],
        reason:
          'Only one day shift is available, which is 6h \u2014 a 12h responder needs two day shifts or one overnight. Ask them to open a second day shift or an overnight.',
      };
    }
    return { patterns: [], reason: 'No combination of available shifts adds up to a full 12h week (two day shifts or one overnight).' };
  }
