...
| **Would rather not work with X** | adds a *Keep apart* rule (**hard**) | no rule |
...
These nudge the schedule and ranking but never block generation (the one
exception is **Keep apart**, listed here beside its "Schedule together"
counterpart but enforced as a **hard rule** — see below):
...
- **Pairing rules.** The **Pairing rules** panel (step 2, below the roster) has
  two sides — one hard, one soft:
  - **Keep apart** *(hard rule)* — pairs who must **never** share a shift. A
    valid schedule never seats a keep-apart pair together, and this rule outranks
    everything except basic coverage. If the keep-apart rules make a complete
    schedule impossible, the app doesn't quietly break them: it says so, names
    the exact pair(s) to reconsider, and shows the complete schedule that
    relaxing one would unlock. In a best-effort partial schedule, any unavoidable
    overlap is still flagged in the weekly grid ("⚠ pair").
  - **Schedule together** *(soft)* — pairs the coordinator would *like* on the same
...
When a single shift can be completed by any one of several people, each
suggested name is **clickable**: selecting a person shows the exact complete
schedule that results if *they* are the one to open up that shift, so different
options can be compared side by side before making a call. When no single change
is enough, the panel instead lists a **minimal set of changes** — several
different people each opening a different shift — that together complete the
week, again with the resulting schedule.
